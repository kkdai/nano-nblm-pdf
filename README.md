# PDF 文字優化工具

使用 Gemini AI 優化 PDF 檔案中的文字品質，提升清晰度和可讀性。

<img width="1906" height="695" alt="image" src="https://github.com/user-attachments/assets/fb24d906-ef02-4f31-ac82-7cca0f653225" />


## 功能特點

- 📄 上傳 PDF 檔案
- 👁️ **預覽模式**：快速處理第一頁測試效果（節省時間和成本）
- 🖼️ 將 PDF 轉換為高解析度圖片 (最高 600 DPI)
- 🎨 支援多種輸出比例 (16:9, 4:3, 3:4, 9:16, 1:1)
- 🤖 使用 Gemini AI 優化每一頁的文字
- 📑 將優化後的圖片重組為新的 PDF
- 📊 **即時進度追蹤**：
  - 即時進度條顯示處理進度
  - 動態計算預估剩餘時間
  - 顯示預估完成時間
  - 每頁處理耗時統計
  - 完整的時間摘要報告
- 🔍 優化前後對比顯示
- 📥 下載優化後的 PDF 檔案

## 系統需求

### 必要軟體

- Python 3.8+
- poppler (pdf2image 的依賴)

### 安裝 poppler

#### macOS
```bash
brew install poppler
```

#### Ubuntu/Debian
```bash
sudo apt-get install poppler-utils
```

#### Windows
下載並安裝 [poppler for Windows](https://github.com/oschwartz10612/poppler-windows/releases)

## 安裝步驟

1. 複製專案
```bash
git clone <repository-url>
cd nano-nblm-pdf
```

2. 建立虛擬環境 (建議)
```bash
python -m venv venv
source venv/bin/activate  # macOS/Linux
# 或
venv\Scripts\activate  # Windows
```

3. 安裝依賴套件
```bash
pip install -r requirements.txt
```

4. 設定環境變數
```bash
cp .env.example .env
# 編輯 .env 檔案，填入您的 Google Cloud API Key
```

或者，您也可以直接在應用程式的側邊欄輸入 API Key。

## 使用方法

1. 啟動 Streamlit 應用程式
```bash
streamlit run app.py
```

2. 在瀏覽器中開啟應用程式 (通常是 http://localhost:8501)

3. 在側邊欄設定參數：
   - 輸入 Google Cloud API Key
   - 選擇輸出比例 (16:9 適合投影片, 3:4 適合文件)
   - 調整圖片解析度 (DPI)
     - 150 DPI: 快速處理，適合測試
     - 300 DPI: 標準品質 (預設)
     - 600 DPI: 最高品質，處理時間較長

4. 上傳 PDF 檔案

5. 選擇處理模式：
   - **👁️ 預覽第一頁**：快速測試優化效果
     - 只處理第一頁
     - 約 10-15 秒完成
     - 可下載優化後的 PNG 圖片
     - 節省 API 調用成本
   - **🚀 處理全部**：處理完整的 PDF
     - 處理所有頁面
     - 可下載完整的優化 PDF

6. 等待處理完成，查看即時進度

7. 查看優化前後對比

8. 下載優化後的檔案

💡 **建議流程**：
1. 先使用「預覽第一頁」測試效果
2. 確認滿意後，再點擊「處理全部」處理完整文件

## API Key 取得

1. 前往 [Google Cloud Console](https://console.cloud.google.com/)
2. 建立或選擇專案
3. 啟用 Vertex AI API
4. 建立服務帳號並下載 JSON 金鑰
5. 將 API Key 設定為環境變數或在應用程式中輸入

## 處理流程

1. **PDF 轉圖片**: 將 PDF 的每一頁轉換為高解析度 PNG 圖片
2. **AI 優化**: 使用 Gemini AI 優化每張圖片的文字品質
3. **圖片轉 PDF**: 將所有優化後的圖片重組為一個新的 PDF
4. **下載檔案**: 提供下載按鈕，取得優化後的 PDF

## 進度追蹤功能

應用程式提供完整的即時進度追蹤，讓你清楚了解處理狀態：

### 即時資訊顯示

- **進度條**：視覺化顯示處理進度 (0-100%)
- **當前頁面**：顯示正在處理第幾頁（例如：第 3/10 頁）
- **預估剩餘時間**：動態計算剩餘處理時間（例如：5 分 30 秒）
- **預估完成時間**：顯示預計完成的時間點（例如：14:35:20）
- **每頁耗時**：顯示每一頁的實際處理時間

### 時間摘要報告

處理完成後會顯示詳細的時間統計：

**完整處理模式**：
```
⏱️ 處理時間摘要：
- 📑 PDF 轉圖片：2.5 秒
- 🤖 AI 優化：125.3 秒（平均每頁 12.5 秒）
- 📄 重組 PDF：3.2 秒
- ⏰ 總耗時：2 分 11 秒
```

**預覽模式**：
```
⏱️ 處理時間摘要：
- 📑 PDF 轉圖片：2.5 秒
- 🤖 AI 優化第一頁：12.5 秒
- ⏰ 總耗時：0 分 15 秒
💡 預估處理全部 10 頁約需：2 分 10 秒
```

### 預估時間的計算方式

1. **第一頁處理時**：顯示「正在計算預估時間...」
2. **第二頁之後**：根據已處理頁面的平均時間計算
   - 平均每頁時間 = 已耗時間 / 已處理頁數
   - 預估剩餘時間 = 平均每頁時間 × 剩餘頁數
3. **動態調整**：每處理完一頁，預估時間會自動更新

### 使用建議

- 📊 **觀察第一頁時間**：第一頁的處理時間可作為參考，預估整份文件的處理時間
- ⏱️ **預覽模式測試**：先用預覽模式了解單頁處理時間，再決定是否處理全部
- 🎯 **調整 DPI**：如果處理時間太長，可以降低 DPI 設定加快速度

## 注意事項

- 處理時間取決於 PDF 頁數、解析度和網路速度
- **建議先使用預覽模式**測試效果，避免浪費 API 額度
- 建議從較低的 DPI (150) 開始測試
- 確保有足夠的磁碟空間儲存暫時檔案
- API 使用可能會產生費用，請注意 Google Cloud 的計費政策
- 每頁處理約需 10-15 秒，處理大型 PDF 可能需要較長時間

## 技術架構

- **Streamlit**: Web 應用程式框架
- **pdf2image**: PDF 轉圖片
- **Pillow**: 圖片處理
- **img2pdf**: 圖片轉 PDF
- **Google Gemini AI**: 文字優化

## 疑難排解

### 錯誤: "Unable to get page count"
- 確認已安裝 poppler
- 檢查 PDF 檔案是否損壞

### 錯誤: "API Key invalid"
- 確認 API Key 正確
- 檢查 Vertex AI API 是否已啟用

### 處理速度慢
- 降低 DPI 設定
- 減少 PDF 頁數
- 檢查網路連線

## 授權

MIT License

## 貢獻

歡迎提交 Issue 和 Pull Request！
